...
3. **Auto import**:
   ...
   - **If beet exits with code 1** and shows a match but doesn't auto-apply (match % below threshold, or missing tracks penalty) — re-run with `beet import ... <<< "a"` to pipe Apply confirmation. This happens when importing partial albums (e.g., 1 track from a 5-track single).
...
4. **Check result**: analyze beet output
   ...
   - If beet exited with code 1 and showed a match with `[A]pply` prompt → re-run with `<<< "a"` (here-string)
