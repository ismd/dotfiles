...
(defun ismd/load-custom-file ()
  "Load Emacs custom file after init."
  (load custom-file 'noerror))
(add-hook 'elpaca-after-init-hook #'ismd/load-custom-file)
...
(defun ismd/set-fonts-for-frame (frame)
  "Set fonts in FRAME (used as hook for daemon mode)."
  (with-selected-frame frame
    (ismd/set-fonts)))
(if (daemonp)
    (add-hook 'after-make-frame-functions #'ismd/set-fonts-for-frame)
  (ismd/set-fonts))

(add-to-list 'default-frame-alist
             `(font . ,(format "%s-%d" ismd/mono-font (/ ismd/font-size 10))))

(defvar ismd/streaming-mode nil
  "Non-nil when streaming mode (larger fonts) is active.")

(defvar ismd/streaming-font-size 160
  "Font size to use in streaming mode.")

(defvar ismd/streaming-variable-font-size 170
  "Variable pitch font size to use in streaming mode.")

(defun ismd/toggle-streaming-mode ()
  "Toggle streaming mode with larger fonts."
  (interactive)
  (setq ismd/streaming-mode (not ismd/streaming-mode))
  (if ismd/streaming-mode
      (setq ismd/font-size ismd/streaming-font-size
            ismd/variable-font-size ismd/streaming-variable-font-size)
    (setq ismd/font-size 130
          ismd/variable-font-size 140))
  (ismd/set-fonts)
  (message "Streaming mode %s (font size: %dpt)"
           (if ismd/streaming-mode "ON" "OFF")
           (/ ismd/font-size 10)))
#+end_src
...
*** Load Theme
#+begin_src emacs-lisp
(defvar theme-name 'modus-vivendi-tinted "The name of the theme to use")
(when (file-exists-p "~/.config/emacs/theme.el")
  (load-file "~/.config/emacs/theme.el"))

(defun ismd/load-default-theme ()
  "Load the default theme after Elpaca finishes initializing."
  (load-theme theme-name t))
(add-hook 'elpaca-after-init-hook #'ismd/load-default-theme)
#+end_src
...
(ismd/load-module "notes")      ;; Org mode, Denote
